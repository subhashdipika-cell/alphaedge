...
import StrategyLabPage from "./components/StoredStrategyLab.jsx";
import {
...
import {
  ...
  getNseHolidayInfo, fetchNseHolidayInfo, fetchAutoPaperTrades,
} from "./data/bridge.js";
...
// Where a record came from. The headless scanner (scripts/scanner.mjs) and the
// manual "Paper trade this" blotter are deliberately SEPARATE track records —
// the rollup keeps them distinguishable instead of silently merging them.
const AUTO_ORIGIN = "Auto-Scanner";
function tradeOrigin(r) {
  if (r?.origin) return r.origin;
  return r?.source === "Auto-Scan" || String(r?.id || "").startsWith("AUTO-")
    ? AUTO_ORIGIN : "Manual";
}
// Scanner records carry no session tag — derive it from the trade's own time and
// asset, the same fallback the Analytics page uses (App.jsx "By Session").
function exportSession(r) {
  return r?.session || marketSession(r?.asset || r?.assetId, r?.timestamp).session || "—";
}
...
function buildMonthlyMarkdown(month, records) {
  ...
  let md = `---\n`
    ...
    + `auto_trades: ${records.filter(r => tradeOrigin(r) === AUTO_ORIGIN).length}\n`
    + `manual_trades: ${records.filter(r => tradeOrigin(r) !== AUTO_ORIGIN).length}\n`
    + `tags:\n  - ${OBSIDIAN_APP}\n  - monthly\n  - trades\n  - ${month}\n`
    + `---\n\n`;
  ...
  md += section("session",   monthlyGroupRows(records, r => exportSession(r)));
  md += section("style",     monthlyGroupRows(records, r => r.style || "—"));
  md += section("origin",    monthlyGroupRows(records, r => tradeOrigin(r)));

  // ₹ P&L per origin — the scanner and the manual blotter are separate track
  // records and must stay separately readable, not just summed.
  const originRs = {};
  resolvedRs.forEach(r => {
    const k = tradeOrigin(r);
    const x = (originRs[k] = originRs[k] || { n: 0, rs: 0 });
    x.n += 1; x.rs += Number(r.pnlRs || 0);
  });
  if (Object.keys(originRs).length) {
    md += `\n## Realized ₹ by origin\n\n| Origin | Resolved | Net ₹ |\n|---|---|---|\n`;
    Object.entries(originRs).sort((a, b) => b[1].rs - a[1].rs).forEach(([k, x]) => {
      md += `| ${mdCell(k)} | ${x.n} | ${x.rs >= 0 ? "+" : ""}${x.rs.toFixed(0)} |\n`;
    });
  }

  md += `\n## Trades\n\n| Date (IST) | Origin | Session | Asset | Strike | Dir | Style | Setup | Conf | Outcome | RR | Net ₹ |\n`
      + `|---|---|---|---|---|---|---|---|---|---|---|---|\n`;
  [...records].sort((a, b) => (a.timestamp || 0) - (b.timestamp || 0)).forEach(r => {
    const strike = r.strike != null ? `${r.strike}${r.direction || ""}` : "—";
    md += `| ${istStamp(r.timestamp)} | ${mdCell(tradeOrigin(r))} | ${mdCell(exportSession(r))} | `
        + `${mdCell(r.asset || r.assetId)} | ${mdCell(strike)} | ${dir(r.bias)} | `
        + `${mdCell(r.style)} | ${mdCell(r.setup)} | `
        + `${r.confidence != null ? r.confidence + "%" : "—"} | ${oc(r)} | `
        + `${r.riskReward != null ? Number(r.riskReward).toFixed(1) : "—"} | `
        + `${r.pnlRs != null ? (Number(r.pnlRs) >= 0 ? "+" : "") + Number(r.pnlRs).toFixed(0) : "—"} |\n`;
  });
...
// in-progress month and re-write already-exported months on demand. Sources are
// the durable archive (not the 30-day history, so full months survive) MERGED
// with the headless scanner's file via the bridge. Returns a
// {ok, written[], failed[]} summary for UI feedback; retries next launch if the
// bridge is offline (the scanner's trades are simply absent from that attempt).
async function exportMonthlyToObsidian(opts = {}) {
  ...
  // Two sources, merged by id: the browser's durable archive (manual paper
  // trades) and the headless scanner's own file, read through the bridge. The
  // scanner is where nearly all trades come from, and it never touches
  // localStorage — without this the rollup would export an almost-empty month.
  const archive = loadTradeArchive();
  let autoTrades = [];
  try {
    const auto = await fetchAutoPaperTrades();
    if (auto && auto.ok !== false && Array.isArray(auto.trades)) {
      autoTrades = auto.trades.filter(t => t && t.timestamp);
    } else if (auto && auto.ok === false) {
      console.warn("[AlphaEdge] scanner trades unavailable for export:", auto.error);
    }
  } catch (e) { console.warn("[AlphaEdge] scanner fetch failed for export:", e); }

  const byId = new Map();
  archive.forEach(r => { if (r && r.id) byId.set(r.id, r); });
  // Scanner wins on conflict: its copy carries the resolved premium-path P&L.
  autoTrades.forEach(t => byId.set(t.id, { ...byId.get(t.id), ...t, origin: AUTO_ORIGIN }));
  const allRecords = [...byId.values()];

  if (!allRecords.length) return { ok: true, written: [], failed: [], note: "no trades to export yet" };

  const current = istMonthKey(Date.now());
  const byMonth = {};
  allRecords.forEach(r => { if (r && r.timestamp) (byMonth[istMonthKey(r.timestamp)] ||= []).push(r); });
