...
if "%APP_READY%"=="1" (
    ...
    if /i "%TRADING_LAB_HIDDEN%"=="1" (
        powershell.exe -NoLogo -NoProfile -Command "Start-Process -FilePath 'cmd.exe' -ArgumentList '/d','/c','npm.cmd run dev -- --port 5001' -WorkingDirectory '%~dp0' -WindowStyle Hidden -RedirectStandardOutput '%LOGDIR%\app.log' -RedirectStandardError '%LOGDIR%\app.err.log'"
    ) else (
